...
static void data_is_ready(unsigned long received_value,
...
static void SnifferAsyncComplete(uv_work_t *req, int status) {
  ...
  Local<Value> argv[] = {Integer::New(isolate, work->value),
...
void Sniffer(const v8::FunctionCallbackInfo<v8::Value> &args) {
  ...
  uv_queue_work(uv_default_loop(), &work->request, SnifferAsync,
                SnifferAsyncComplete);
