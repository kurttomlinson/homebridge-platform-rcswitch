...
static char* rcswitch_dec2bin_wzerofill(unsigned long dec,
                                        unsigned int bit_length);
static void rcswitch_transmit(int pin, int pulse, int nHighPulses,
                              int nLowPulses);
...
static char isWiringSetup;
void rcswitch_init() { rcswitch_set_receive_tolerance(60); }
...
static void rcswitch_handle_interrupt() {
  ...
  if (duration > 5000 && duration > timings[0] - 200 &&
      duration < timings[0] + 200) {
    repeatCount++;
    changeCount--;

    if (repeatCount == 2) {
      if (!rcswitch_receive_protocol1(changeCount) &&
          !rcswitch_receive_protocol2(changeCount)) {
        // failed
      }
      repeatCount = 0;
    }
...
static int rcswitch_receive_protocol1(unsigned int changeCount) {
  ...
  for (i = 1; i < changeCount; i += 2) {
    if (timings[i] > delay - delayTolerance &&
        ...
        timings[i + 1] > delay * 3 - delayTolerance &&
        timings[i + 1] < delay * 3 + delayTolerance) {
      code = code << 1;
    } else if (timings[i] > delay * 3 - delayTolerance &&
               timings[i] < delay * 3 + delayTolerance &&
  ...
  code = code >> 1;
  if (changeCount > 6 && code &&
      data_ready_cb) {  // ignore < 4bit values as there are no devices sending
                        // 4bit values => noise
...
static int rcswitch_receive_protocol2(unsigned int changeCount) {
  ...
  for (i = 1; i < changeCount; i += 2) {
    if (timings[i] > delay - delayTolerance &&
        ...
        timings[i + 1] < delay * 2 + delayTolerance) {
      code = code << 1;
    } else if (timings[i] > delay * 2 - delayTolerance &&
               timings[i] < delay * 2 + delayTolerance &&
               timings[i + 1] > delay - delayTolerance &&
               timings[i + 1] < delay + delayTolerance) {
      code += 1;
      code = code << 1;
...
void rcswitch_send(int protocol, int pin, int repeat, int pulse,
                   unsigned long code, unsigned int length) {
  rcswitch_send_scode_word(protocol, pin, repeat, pulse,
                           rcswitch_dec2bin_wzerofill(code, length));
}

void rcswitch_send_scode_word(int protocol, int pin, int repeat, int pulse,
                              char* sCodeWord) {
  if (!isWiringSetup) {
    if (wiringPiSetup() == -1) {
